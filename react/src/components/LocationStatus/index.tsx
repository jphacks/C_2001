...
export const LocationStatus = () => {
  const { requestNotice, toggleStatusFn } = useListenReqNoticeUsecase();

  const StatusSelectButton = (
    judgStatus: "private" | "public",
    props: string
  ) => (
    <div
      onClick={() => {
        toggleStatusFn(judgStatus);
      }}
    >
      {requestNotice?.userStatus.type === judgStatus ? (
        <Koukai1>
          <p>{props}</p>
        </Koukai1>
      ) : (
  ...
  return (
    <div>
      ...
      {requestNotice?.userStatus.type === "request" ||
      ...
      <Range>
        ...
        {StatusSelectButton("public", "友だち全員に公開")}
        {StatusSelectButton("private", "誰にも公開しない(非公開)")}
      </Range>
